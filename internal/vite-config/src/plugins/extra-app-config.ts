...
const BASIC_ADMIN_PRO_APP_CONF = '_BASIC_ADMIN_PRO_APP_CONF_';
...
async function getConfigSource() {
  ...
  const windowVariable = `window.${BASIC_ADMIN_PRO_APP_CONF}`;
  // 确保变量不会被修改
  let source = `${windowVariable}=${JSON.stringify(config)};`;
  source += `
    Object.freeze(${windowVariable});
    Object.defineProperty(window, "${BASIC_ADMIN_PRO_APP_CONF}", {
      configurable: false,
      writable: false,
    });
  `.replaceAll(/\s/g, '');
